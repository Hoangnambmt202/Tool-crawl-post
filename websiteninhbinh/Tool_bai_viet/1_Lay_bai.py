...
# ROTATE_UA_EVERY: import mềm — nếu chưa thêm vào config.py thì mặc định 0 (tắt)
try:
    from config import ROTATE_UA_EVERY
except ImportError:
    ROTATE_UA_EVERY = 0
...
def normalize_img_srcs_in_content(tag_content, base_url: str = ""):
    ...
    Xử lý cả lazy-load (data-src, data-original, data-lazy-src, data-original-src,
    data-bg, srcset) và inline style background.
    ...
    _LAZY_ATTRS = (
        ...
        'data-lazy-src', 'data-original-src', 'data-url', 'data-bg',
    )

    for img in tag_content.find_all('img'):
        # Xử lý các attr src thông thường và lazy-load
        ...
        # Xử lý srcset — mỗi entry dạng "url width" hoặc chỉ "url"
...
def clean_html_content(tag_content):
    ...
    for tag in tag_content.find_all(True):
        ...
        if tag.name == 'a':
            ...
            src    = tag.get('src') or ''
            # Giữ lại lazy-load attrs (đã được normalize thành absolute URL)
            # để script đăng bài có thể upload đúng ảnh
            _KEEP_IMG_ATTRS = (
                'src', 'data-src', 'data-original', 'data-lazy',
                'data-lazy-src', 'data-original-src', 'data-url', 'srcset',
            )
            kept = {a: tag.get(a) for a in _KEEP_IMG_ATTRS if tag.get(a)}
            tag.attrs = {}
            tag.attrs.update(kept)
            if not tag.get('src') and src:
                tag['src'] = src
...
# ── Config chống ban — import mềm ────────────────────────────────────────────
try:
    from config import SCRAPE_DELAY_MIN, SCRAPE_DELAY_MAX
except ImportError:
    SCRAPE_DELAY_MIN = 1.5   # giây tối thiểu giữa 2 request
    SCRAPE_DELAY_MAX = 4.0   # giây tối đa

try:
    from config import BAN_COOLDOWN_SEC
except ImportError:
    BAN_COOLDOWN_SEC = 120   # nghỉ bao nhiêu giây khi bị 403/CAPTCHA

try:
    from config import BAN_MAX_RETRIES
except ImportError:
    BAN_MAX_RETRIES = 3      # thử lại tối đa N lần sau mỗi lần bị ban


def _human_delay(min_s: float = None, max_s: float = None) -> float:
    """
    Delay ngẫu nhiên mô phỏng hành vi người dùng thật.
    Phân phối lệch phải (người dùng thường nhanh nhưng thỉnh thoảng chậm hẳn):
      • 80% thời gian: trong khoảng [min_s, min_s + (max_s-min_s)*0.5]
      • 20% thời gian: trong khoảng trên đến max_s (đọc kỹ, bị phân tâm)
    Trả về số giây đã sleep.
    """
    lo = min_s if min_s is not None else SCRAPE_DELAY_MIN
    hi = max_s if max_s is not None else SCRAPE_DELAY_MAX
    mid = lo + (hi - lo) * 0.5
    if random.random() < 0.80:
        delay = random.uniform(lo, mid)
    else:
        delay = random.uniform(mid, hi)
    time.sleep(delay)
    return delay


def _make_chrome_options(ua: str) -> Options:
    """
    Tạo ChromeOptions với đầy đủ stealth flags.
    Gọi mỗi khi khởi tạo hoặc restart driver để đảm bảo UA mới được áp dụng.

    Stealth checklist:
      • User-Agent ngẫu nhiên từ danh sách thật
      • Tắt AutomationControlled blink feature
      • Ẩn 'enable-automation' switch khỏi CDP
      • Tắt useAutomationExtension
      • Window size ngẫu nhiên (tránh fingerprint cố định)
      • Timezone, locale phù hợp website VN
      • Tắt các tính năng Chrome lộ automation

    LƯU Ý: excludeSwitches + useAutomationExtension không có tác dụng với
    --headless. Nếu cần headless, dùng --headless=new thay vì --headless cũ.
    """
    opts = Options()
    opts.binary_location = CHROME_BINARY

    # ── Stealth flags cốt lõi ─────────────────────────────────────────────
    opts.add_argument(f"--user-agent={ua}")
    opts.add_argument("--disable-blink-features=AutomationControlled")
    opts.add_experimental_option("excludeSwitches", ["enable-automation"])
    opts.add_experimental_option("useAutomationExtension", False)

    # ── Locale / timezone phù hợp website VN ─────────────────────────────
    opts.add_argument("--lang=vi-VN")
    opts.add_experimental_option("prefs", {
        "intl.accept_languages": "vi-VN,vi,en-US,en",
    })

    # ── Tắt các tính năng dễ bị nhận diện automation ─────────────────────
    opts.add_argument("--disable-extensions")
    opts.add_argument("--disable-infobars")
    opts.add_argument("--disable-notifications")
    opts.add_argument("--disable-popup-blocking")
    # Bỏ banner "Chrome đang được điều khiển bởi phần mềm tự động"
    opts.add_argument("--disable-translate")

    # ── Window size ngẫu nhiên quanh 1366×768 (phổ biến nhất VN) ─────────
    _sizes = [
        (1366, 768), (1440, 900), (1536, 864),
        (1280, 800), (1600, 900), (1920, 1080),
    ]
    w, h = random.choice(_sizes)
    w += random.randint(-10, 10)   # ±10px noise
...
# ── Stealth JS toàn diện — inject vào MỖI trang mới ─────────────────────────

_STEALTH_JS = r"""
// ── 1. Ẩn webdriver flag ────────────────────────────────────────────────────
Object.defineProperty(navigator, 'webdriver', {get: () => false});

// ── 2. Plugins giống trình duyệt thật ───────────────────────────────────────
const _plugins = [
...
// ── 4. Platform / hardware concurrency ───────────────────────────────────────
Object.defineProperty(navigator, 'platform',            {get: () => 'Win32'});
Object.defineProperty(navigator, 'hardwareConcurrency', {get: () => 4});
Object.defineProperty(navigator, 'deviceMemory',        {get: () => 8});

// ── 5. Screen — phù hợp với --window-size ────────────────────────────────────
// Không override screen (Chrome tự set đúng theo --window-size)

// ── 6. Xóa dấu vết ChromeDriver (cdc_*, $cdc_*) ─────────────────────────────
Object.getOwnPropertyNames(window)
    .filter(k => /^(cdc_|_cdc)/.test(k))
    .forEach(k => { try { delete window[k]; } catch(e){} });

// ── 7. Permissions API — tránh fingerprint "notification denied by bot" ───────
...
// ── 8. WebGL vendor/renderer — giá trị người dùng thật ──────────────────────
...
// ── 9. Canvas fingerprint noise (±1 bit mỗi pixel, không thể phân biệt) ─────
const _toBlob  = HTMLCanvasElement.prototype.toBlob;
const _toDataURL = HTMLCanvasElement.prototype.toDataURL;
const _getImageData = CanvasRenderingContext2D.prototype.getImageData;
function _addNoise(data) {
    for (let i = 0; i < data.length; i += 4) {
        data[i]   ^= Math.random() < 0.01 ? 1 : 0;
        data[i+1] ^= Math.random() < 0.01 ? 1 : 0;
        data[i+2] ^= Math.random() < 0.01 ? 1 : 0;
    }
}
CanvasRenderingContext2D.prototype.getImageData = function(...args) {
    const d = _getImageData.apply(this, args);
    _addNoise(d.data);
    return d;
};
"""
...
def _patch_stealth(driver) -> None:
    """
    Inject _STEALTH_JS vào mỗi trang mới qua CDP addScriptToEvaluateOnNewDocument.
    Bao gồm: webdriver flag, plugins, languages, platform, hardware,
    ChromeDriver keys, Permissions API, WebGL, Canvas noise.
    """
    try:
        ...
        log.warning(f"_patch_stealth: không áp dụng được CDP patch — {_e}")


def _human_scroll(driver, total_px: int = None) -> None:
    """
    Scroll trang theo kiểu người dùng thật:
    cuộn từng đoạn nhỏ với tốc độ và độ dài ngẫu nhiên,
    thỉnh thoảng dừng lại (đọc), đôi khi cuộn ngược lại một chút.
    Không raise exception — lỗi JS được bỏ qua im lặng.
    """
    try:
        page_h = driver.execute_script("return document.body.scrollHeight") or 3000
        if total_px is None:
            # Cuộn xuống 40–80% chiều cao trang (người dùng không đọc hết)
            total_px = int(page_h * random.uniform(0.4, 0.8))

        current = 0
        while current < total_px:
            step = random.randint(120, 380)   # mỗi lần cuộn 120-380px
            current += step
            driver.execute_script(f"window.scrollBy(0, {step});")

            # Thỉnh thoảng cuộn ngược (đọc lại)
            if random.random() < 0.10:
                back = random.randint(30, 100)
                driver.execute_script(f"window.scrollBy(0, -{back});")
                current -= back

            # Dừng ngắn giữa các bước cuộn
            time.sleep(random.uniform(0.05, 0.25))

            # 15% cơ hội dừng lâu hơn (đọc đoạn này)
            if random.random() < 0.15:
                time.sleep(random.uniform(0.4, 1.2))

    except Exception:
        pass   # JS lỗi không làm crash scraper


def _human_mouse_jitter(driver) -> None:
    """
    Giả lập rê chuột nhỏ qua JS (di chuyển con trỏ không có ý nghĩa).
    Không phụ thuộc selenium-wire / pyautogui.
    """
    try:
        driver.execute_script("""
            const evt = new MouseEvent('mousemove', {
                bubbles: true,
                clientX: Math.floor(Math.random() * window.innerWidth  * 0.6 + 100),
                clientY: Math.floor(Math.random() * window.innerHeight * 0.5 + 80),
            });
...
_ban_counters: dict = {}   # base_url → số lần bị ban liên tiếp


def _detect_ban(html: str, url: str) -> str | None:
    """
    Phát hiện soft-ban / CAPTCHA ngoài các lỗi HTTP thông thường.
    Trả về chuỗi mô tả nếu bị ban, None nếu bình thường.
    """
    if not html:
        return None
    h = html[:4000].lower()
    checks = [
        (r'<title>[^<]*(captcha|robot|security check|cloudflare)[^<]*</title>',
         "CAPTCHA/Bot check trong <title>"),
        (r'(g-recaptcha|hcaptcha|cf-turnstile)',
         "Widget CAPTCHA trong body"),
        (r'(just a moment|checking your browser|please wait|ddos-guard)',
         "Trang chờ chống DDoS"),
        (r'(access denied|you have been blocked|your ip|ip address.*blocked)',
         "IP bị chặn"),
        (r'(too many requests|rate limit|slow down)',
...
def _handle_ban(base_url: str, reason: str, driver) -> bool:
    """
    Xử lý khi bị ban:
      1. Tăng counter ban cho site này
      2. Nghỉ BAN_COOLDOWN_SEC giây (có thể đổi UA sau đó)
      3. Trả về True nếu nên tiếp tục, False nếu đã vượt BAN_MAX_RETRIES

    Gọi từ extract_data khi _detect_ban() trả về kết quả.
    """
    count = _ban_counters.get(base_url, 0) + 1
    _ban_counters[base_url] = count

    log.warning(
        f"🚫 Bị ban [{reason}] tại {base_url} "
        f"(lần {count}/{BAN_MAX_RETRIES}) — nghỉ {BAN_COOLDOWN_SEC}s..."
    )

    if count > BAN_MAX_RETRIES:
        log.critical(f"✗ Vượt quá {BAN_MAX_RETRIES} lần bị ban tại {base_url} — bỏ qua site")
        return False

    # Nghỉ dài, thỉnh thoảng tải trang khác để "làm ấm" cookie
    time.sleep(BAN_COOLDOWN_SEC * random.uniform(0.8, 1.2))

    # Thử tải trang chủ của site (reset session nhẹ)
    try:
        driver.get(base_url)
...
class VspProducts:
    def __init__(self, base, url, cat, target):
        ...
        log.info(f"  User-Agent: {self._current_ua[:80]}")
        service        = Service(CHROMEDRIVER_PATH)
        chrome_options = _make_chrome_options(self._current_ua)

        try:
            self.driver = webdriver.Chrome(service=service, options=chrome_options)
            _patch_stealth(self.driver)
            ...
            if not SHOW_CHROME_WINDOW:
                try:
                    ...
                    try:
                        ...
                        pass
            log.success("✓ Chrome driver sẵn sàng (stealth mode)")
        except Exception as e:
        ...
        self._detail_count = 0  # đếm bài chi tiết đã xử lý — dùng để rotate UA
    ...
    def reset_for_row(self):
        ...
        # _detail_count KHÔNG reset — cần tích lũy xuyên phiên để rotate UA đúng nhịp
    ...
    def _extract_thumbnail_url(tag, base: str = "") -> str:
        ...
        # Thứ tự attr lazy-load ưu tiên — cộng thêm data-lazy-src, data-original-src,
        # data-bg, srcset (lấy URL đầu tiên) so với phiên bản cũ
        _LAZY_ATTRS = (
            'src', 'data-src', 'data-original', 'data-lazy',
            'data-lazy-src', 'data-original-src', 'data-url', 'data-bg',
        )
        ...
        for selector in (
            ...
            src = ''
            # 1. Thử từng lazy-load attr theo thứ tự ưu tiên
            for attr in _LAZY_ATTRS:
                val = (img.get(attr) or '').strip()
                _vl = val.lower()
                _is_placeholder = (
                    ...
                    or any(_p in _vl for _p in ('blank.gif', 'transparent.gif',
                                                 '1x1.gif', 'loading.gif',
                                                 'spacer.gif', 'placeholder',
                                                 'data:image/gif'))
                )
                if val and not _is_placeholder:
                    src = val
                    break

            # 2. Fallback: srcset — lấy URL đầu tiên (trước khoảng trắng)
            if not src:
                srcset = (img.get('srcset') or '').strip()
                if srcset:
                    src = srcset.split()[0]   # "url1 1x, url2 2x" → "url1"
            ...
            # Chuyển tất cả dạng relative → absolute (bao gồm cả "uploads/img.jpg")
            if src.startswith('//'):
                src = 'https:' + src
            elif base and not src.startswith(('http://', 'https://', 'data:')):
    ...
    def _smart_add(self, name: str, url: str, cat_id: str,
        ...
        _url_set = {u for u, _ in self.url_links}   # O(1) lookup thay vì O(n)
        if url in _url_set:
        ...
        def _norm_name(s: str) -> str:
            """NFKC + lowercase + strip — tránh NFD/NFC mismatch tiếng Việt."""
            return _ucd.normalize("NFKC", (s or "")).strip().lower()

        _name_norm = _norm_name(name)
        existing_cam = next(
            (c for c in self.camlist.camobs if _norm_name(c.name) == _name_norm), None)
        ...
        if existing_cam:
            ...
            if is_better(new_score, old_score):
                ...
                self.camlist.camobs = [
                    c for c in self.camlist.camobs if _norm_name(c.name) != _name_norm]
                new_cam = camob.CameraObject(0, name, 0, url, "", cat_id)
    ...
    def _try_save_cam(self, cam, o_url: str, t_start, struct_type,
        ...
        if already_exists:
            print(f"  -> đã tồn tại trong DB (title+content trùng)")
        ...
        # Guard: không lưu bài không có tiêu đề lẫn nội dung (tránh rác vào DB)
        if not title and not content:
            log.warning("Bỏ qua lưu — cam không có title lẫn nội dung")
            elapsed = (datetime.now() - t_start).total_seconds()
            stats.record_url(o_url, struct_type, success=False,
                             error_msg="title+content rỗng")
            return True
    ...
    def restart_driver(self):
        """Khởi động lại Chrome driver sau khi crash — đổi User-Agent mới."""
        ...
        # Rotate sang UA mới (tránh bị block theo UA cũ)
        new_ua = random.choice(USER_AGENTS)
        while new_ua == self._current_ua and len(USER_AGENTS) > 1:
            new_ua = random.choice(USER_AGENTS)
        self._current_ua = new_ua
        log.info(f"  User-Agent mới: {new_ua[:80]}")

        service        = Service(CHROMEDRIVER_PATH)
        chrome_options = _make_chrome_options(new_ua)
        try:
            self.driver = webdriver.Chrome(service=service, options=chrome_options)
            _patch_stealth(self.driver)
            if not SHOW_CHROME_WINDOW:
            ...
            log.success("✓ Driver đã khởi động lại (UA mới, stealth mode)")
        except Exception as e:
    ...
    def get_data(self):
        ...
        try:
            ...
            _human_scroll(self.driver, total_px=random.randint(200, 600))
        ...
        while self.url_links:
            ...
            # Kiểm tra trước xem đây có phải bài chi tiết không
            # (cùng logic với biến `chitiet` bên trong extract_data)
            # Fallback: nếu camlist rỗng (URL trực tiếp từ Excel, không qua listing)
            # → dùng heuristic: URL có extension .html hoặc chứa số ID ở cuối
            _is_detail = any(
                self._norm_url(cam.url) == self._norm_url(current_url)
                for cam in self.camlist.camobs
            )
            if not _is_detail and not self.camlist.camobs:
                # Heuristic: URL trực tiếp (không qua listing) thường có dạng
                # /bai-viet-abc-12345.html hoặc ?id=123 — coi như chi tiết
                _is_detail = bool(
                    re.search(r'-\d{3,}(?:\.html?)?(?:[?#]|$)', current_url)
                    or re.search(r'[?&](?:p|id|post_id)=\d+', current_url)
                )
            ...
            # ── Rotate UA sau mỗi ROTATE_UA_EVERY bài chi tiết ───────────────
            if _is_detail and ROTATE_UA_EVERY > 0:
                self._detail_count += 1
                if self._detail_count % ROTATE_UA_EVERY == 0:
                    log.warning(
                        f"🔄 Rotate UA sau {self._detail_count} bài "
                        f"(mỗi {ROTATE_UA_EVERY} bài) — restart driver..."
                    )
                    try:
                        self.restart_driver()
                    except Exception as _re:
                        log.critical(f"Rotate UA thất bại: {_re} — tiếp tục với driver cũ")
    ...
    def extract_data(self, o_url: str):
        ...
        _human_delay()   # delay ngẫu nhiên, không cố định

        # ── Tải trang ────────────────────────────────────────────────────────
        _load_err_msg = ""
        _ban_retry    = 0
        while True:   # vòng retry khi bị soft-ban
            try:
                self.driver.get(o_url)
                _human_delay(0.8, 2.0)   # chờ JS render xong
                _human_mouse_jitter(self.driver)
            ...
            # ── Detect soft-ban / CAPTCHA ─────────────────────────────────
            ban_reason = _detect_ban(html, o_url)
            if ban_reason:
                _ban_retry += 1
                should_continue = _handle_ban(get_base(o_url), ban_reason, self.driver)
                if not should_continue or _ban_retry >= BAN_MAX_RETRIES:
                    _load_err_msg = f"Bị ban: {ban_reason}"
                    break
                # Rotate UA sau khi bị ban rồi thử lại
                try:
                    self.restart_driver()
                except Exception:
                    _load_err_msg = "Restart driver thất bại sau ban"
                    break
                continue   # thử lại sau khi restart
        ...
        while True:
            ...
            # Scroll ngẫu nhiên (listing: cuộn nhiều; chi tiết: cuộn vừa)
            _human_scroll(self.driver,
                          total_px=random.randint(600, 1800) if not chitiet
            ...
            if chitiet:
                ...
                for cam in self.camlist.camobs:
                    if self._norm_url(cam.url) == o_url:
                        ...
                        # ── Ảnh đại diện: lấy TRƯỚC clean_html_content để không mất src ──
                        # (normalize_img_srcs_in_content đã chạy → src đã là absolute)
                        _thumb = getattr(cam, '_thumbnail_url', '')
                        if not _thumb:
                            _first_img = tag_content.find('img')
                            if _first_img:
                                # Ưu tiên lazy-load attrs trước khi clean xoá chúng
                                for _la in ('src', 'data-src', 'data-original',
                                            'data-lazy', 'data-lazy-src', 'data-url'):
                                    _src = (_first_img.get(_la) or '').strip()
                                    if _src and not _src.startswith('data:') and \
                                            'no-image' not in _src.lower():
                                        # resolve bất kỳ relative URL còn sót
                                        if _src.startswith('//'):
                                            _src = 'https:' + _src
                                        elif not _src.startswith(('http://', 'https://')):
                                            _src = urljoin(self.base, _src)
                                        _thumb = _src
                                        break
                        # Fallback: thumbnail từ YouTube embed
                        if not _thumb:
            ...
            # ════════════════════════════════════════════════════════════════
            if chitiet:
                _human_delay(1.0, 2.5)   # đọc trang trước khi chuyển
                # Reset ban counter khi tải thành công
                _ban_counters.pop(get_base(o_url), None)
                elapsed = (datetime.now() - t_start).total_seconds()
                stats.record_url(o_url, detected_structure)
            ...
            if btns:
                ...
                _human_delay(0.3, 0.8)   # hover trước khi click
                self.driver.execute_script("arguments[0].click();", b)
                _human_delay(1.5, 3.5)   # chờ trang tiếp tải
                html = self.driver.page_source
                _page_num += 1
...
def run_from_excel(
    ...
    try:
        for r in range(start_row, real_last_row + 1):
            ...
            # Delay giữa các dòng Excel — ngẫu nhiên (không đều đặn)
            _human_delay(sleep_between, sleep_between * 2.5)
