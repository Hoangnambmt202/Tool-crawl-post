...
class VspProducts:
    def __init__(self, base, url, cat,target):
        ...
        service = Service(r"D:\folder\chromedriver-win64\chromedriver-win64\chromedriver.exe")
        chrome_options = Options()
        chrome_options.binary_location = r"D:\folder\chrome-win64\chrome-win64\chrome.exe"
        ...
        if soup:
            ...
            while True:
                ...
                if chitiet and tag_title_product  :
                    ...
                    # # 2) fallback: lấy "Ngày ban hành" trong module34
                    # if d is None:
                    #     d = parse_issue_date_from_module34(soup)

                    # # 3) nếu vẫn None thì bạn quyết định:
                    # # - hoặc bỏ qua lọc FROM_DATE
                    # # - hoặc gán date.today() / hoặc None (tuỳ nghiệp vụ)
                    if  d is not None :
                        print(d < FROM_DATE)
                        if d < FROM_DATE:
                            print(f"Bài {d} < {FROM_DATE} => bỏ qua")
...
# chạy
run_from_excel(r"D:\folder\websiteninhbinh\danhsachweb.xlsx", sheet_name=None)
